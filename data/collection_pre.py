...
def get_collection(file_path, save_suffix):
    try:
        ...
        # df.columns = ['cve', 'owner', 'repo', 'commit_id', 'label', 'desc_token', 'msg_token', 'diff_token']
        reduce_mem_usage(df)
        ...
        desc_token = pd.DataFrame({'qid': range(0, len(desc_token)), 'desc_token': desc_token})
        ...
        combined = pd.DataFrame({'cid': range(0, len(combined)), 'commits_token': combined})
